...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/quirkyturtcoinParams"
    else
        PARAMS_DIR="$HOME/.quirkyturtcoin-params"
    fi
fi
...
Failed to fetch the quirkyturtcoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
quirkyturtcoin - fetch-params.sh

This script will fetch the quirkyturtcoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common quirkyturtcoin zkSNARK parameters. Note that it is
