...
#if defined(NDEBUG)
#error "quirkyturtcoin cannot be compiled without assertions."
#endif
...
{
    util::ThreadRename("quirkyturtcoin-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // quirkyturtcoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nquirkyturtcoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nquirkyturtcoinBadBlockBits)) {
            // accept quirkyturtcoin block minted with incorrect proof of work threshold
            return true;
        }
