...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("quirkyturtcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("quirkyturtcoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("quirkyturtcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("quirkyturtcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running quirkyturtcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("quirkyturtcoinMiner exception");
    } catch (...) {
        LogPrintf("quirkyturtcoinMiner exception");
    }

    LogPrintf("quirkyturtcoinMiner exiting\n");
}
