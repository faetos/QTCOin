...
const char * const quirkyturtcoin_CONF_FILENAME = "quirkyturtcoin.conf";
const char * const quirkyturtcoin_PID_FILENAME = "quirkyturtcoin.pid";
const char * const quirkyturtcoin_MASTERNODE_CONF_FILENAME = "masternode.conf";


// quirkyturtcoin only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "quirkyturtcoind -foo=bar
...
#else
    const char* pszModule = "quirkyturtcoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\quirkyturtcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\quirkyturtcoin
// Mac: ~/Library/Application Support/quirkyturtcoin
// Unix: ~/.quirkyturtcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "quirkyturtcoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\quirkyturtcoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\quirkyturtcoinParams
    // Mac: ~/Library/Application Support/quirkyturtcoinParams
    // Unix: ~/.quirkyturtcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "quirkyturtcoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "quirkyturtcoinParams";
#else
    // Unix
    return pathRet / ".quirkyturtcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/quirkyturtcoin/sapling-spend.params";
        sapling_output = "/usr/share/quirkyturtcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/quirkyturtcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/quirkyturtcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", quirkyturtcoin_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", quirkyturtcoin_PID_FILENAME));
