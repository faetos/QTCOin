...
#define URI_SCHEME "quirkyturtcoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter quirkyturtcoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no quirkyturtcoin: URI
...
{
    // Convert quirkyturtcoin:// to quirkyturtcoin:
    //
    //    Cannot handle this later, because quirkyturtcoin:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", quirkyturtcoin_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "quirkyturtcoin (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "quirkyturtcoin (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "quirkyturtcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for quirkyturtcoin*.lnk
...
{
    return GetAutostartDir() / "quirkyturtcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a quirkyturtcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=quirkyturtcoin (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=quirkyturtcoin (regtest)\n";
        else
            optionFile << "Name=quirkyturtcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the quirkyturtcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add quirkyturtcoin app to startup item list
