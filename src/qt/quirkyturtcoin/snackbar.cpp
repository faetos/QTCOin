...
#include "qt/quirkyturtcoin/forms/ui_snackbar.h"
#include "qt/quirkyturtcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(quirkyturtcoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &quirkyturtcoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
