// Copyright (c) 2019 The quirkyturtcoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "qt/quirkyturtcoin/settings/settingsfaqwidget.h"
#include "qt/quirkyturtcoin/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/quirkyturtcoin/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(quirkyturtcoinGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("quirkyturtcoin is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("quirkyturtcoin utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableQRTCContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received quirkyturtcoin requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your quirkyturtcoin wallet also needs to be completely synchronized "
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of quirkyturtcoin with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("quirkyturtcoin-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.quirkyturtcoin.org'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node quirkyturtcoin core wallet with a "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in quirkyturtcoin Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the quirkyturtcoin network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    if (parent)
        connect(parent, &quirkyturtcoinGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
