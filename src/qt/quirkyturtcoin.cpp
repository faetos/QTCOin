...
#include "config/quirkyturtcoin-config.h"
#endif
...
#include "qt/quirkyturtcoin/splash.h"
#include "qt/quirkyturtcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("quirkyturtcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in quirkyturtcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in quirkyturtcoin.qrc)
...
/** Class encapsulating quirkyturtcoin Core startup and shutdown.
...
/** Main quirkyturtcoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (quirkyturtcoinGUI)
...
private:
    ...
    quirkyturtcoinGUI* window{nullptr};
...
{
    window = new quirkyturtcoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &quirkyturtcoinGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &quirkyturtcoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(quirkyturtcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(quirkyturtcoinGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // quirkyturtcoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &quirkyturtcoinGUI::handlePaymentRequest);
        connect(window, &quirkyturtcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. quirkyturtcoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(quirkyturtcoin_locale);
    Q_INIT_RESOURCE(quirkyturtcoin);
    ...
    /// 6. Determine availability of data and blocks directory and parse quirkyturtcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("quirkyturtcoin Core"),
            QObject::tr("Error: Specified data directory \"%1\" does not exist.").arg(QString::fromStdString(gArgs.GetArg("-datadir", ""))));
        return 1;
    }
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", quirkyturtcoin_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr("quirkyturtcoin Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("quirkyturtcoin Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // quirkyturtcoin: links repeatedly have their payment requests routed to this process:
