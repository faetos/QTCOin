...
// Copyright (c) 2021 The quirkyturtcoin Core developers
...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The quirkyturtcoin address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The quirkyturtcoin address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The quirkyturtcoin address to use for masternode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The quirkyturtcoin address to use for masternode reward payments.\n"
...
// Allows to specify quirkyturtcoin address or priv key (as strings). In case of quirkyturtcoin address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid quirkyturtcoin address %s", strAddress));
    }
    return *keyID;
}
