...
- [Development guidelines](#development-guidelines)
    - [General quirkyturtcoin Core](#general-quirkyturtcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
quirkyturtcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
quirkyturtcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/quirkyturtcoin-project/quirkyturtcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_quirkyturtcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_quirkyturtcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/quirkyturtcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_quirkyturtcoin.coverage/index.html`.
```
...
However, in open source software such as quirkyturtcoin Core, where everyone uses
...
pay attention to for reviewers of quirkyturtcoin Core code.

General quirkyturtcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: quirkyturtcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against quirkyturtcoin Core so that they can be integrated
...
file descriptors it will cause problems with quirkyturtcoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:quirkyturtcoin-Project/quirkyturtcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `quirkyturtcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `quirkyturtcoin-cli` and the GUI debug console use this table to determine how to
